...
import { Package, ArrowRight, TrendingUp, ChevronDown, ChevronUp } from 'lucide-react';
...
const LiveShipmentTracker: React.FC = () => {
    ...
    const [expandedIds, setExpandedIds] = useState<Set<string>>(new Set(['SHP-992-NY']));
    ...
    return (
        <div className="tracker-container">
            <div className="tracker-header">
                <h3><Package size={20} color="var(--color-indigo)" /> Live Shipment Tracker</h3>
            ...
            <div className="shipment-list">
                {shipments.map(shipment => {
                    ...
                    return (
                        ...
                        >
                            <div className="shipment-top-row" style={{ marginBottom: isExpanded ? '1rem' : '0' }}>
                                <div>
                            ...
                            {isExpanded && (
                                <div className="shipment-details slide-down-fast">
                                    ...
                                    <div className="pizza-tracker">
                                        ...
                                        <div className="pizza-tracker-steps">
                                            {TRACKER_STEPS.map((step, index) => {
                                                const stepThreshold = (index / (TRACKER_STEPS.length - 1)) * 100;
                                                ...
                                                const isActive = !isCompleted && shipment.progress >= (stepThreshold - (100 / (TRACKER_STEPS.length - 1)));

                                                return (
                                                    <div key={step} className={`tracker-step ${isCompleted ? 'completed' : ''} ${isActive ? 'active' : ''}`}>
                                                        <div className="step-dot">
                                                            {isCompleted && <div className="dot-inner-check" />}
                                                        </div>
                                    ...
                                    <div className="eta-label">
                                        <span>Estimated Arrival: <span style={{ color: 'var(--text-primary)' }}>{shipment.eta}</span></span>
                                    </div>
                                </div>
                            )}
                        </div>

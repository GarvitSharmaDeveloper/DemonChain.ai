...
const DatadogDashboard: React.FC<DatadogDashboardProps> = ({
  isCrisis,
  severity = "Critical",
}) => {
  ...
  useEffect(() => {
    if (isCrisis) {
      ...
      const timer = setTimeout(() => {
        const spikeAmount =
          severity === "Critical" ? 14 : severity === "High" ? 8 : 3;
        setDelayDays(spikeAmount);
        setRiskScore(
          severity === "Critical" ? 98 : severity === "High" ? 85 : 65,
        );
        setChartData((prev) => {
    ...
    <div className="dd-dashboard">
      ...
      <div className="dd-grid-top">
        <div className="dd-widget">
          ...
          <div
            className={classNames("widget-value", { alert: riskScore > 50 })}
          >
            {riskScore}/100
          </div>
        </div>
      ...
      <div className="dd-chart-container">
        ...
        >
          ...
          <path
            d={areaD}
            className={classNames("chart-area", { alert: delayDays > 0 })}
          />
          <path
            d={pathD}
            className={classNames("chart-line", { alert: delayDays > 0 })}
          />
        </svg>
      </div>
      ...
      <div className="dd-widget" style={{ flex: "0 0 auto" }}>
        ...
        <div className="dd-apm-list">
          ...
          {isCrisis && (
            <div
              className="apm-row slide-down"
              style={{ borderLeft: "3px solid var(--color-red)" }}
            >
              <span>POST /api/v1/events</span>
              <span className="apm-status error">
                202 ACCEPTED (Alert Fired)
              </span>
            </div>
          )}
          {datadogEventId && (

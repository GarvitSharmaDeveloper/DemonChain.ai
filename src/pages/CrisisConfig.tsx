...
const CrisisConfig: React.FC<CrisisConfigProps> = ({ setIsCrisis, setCrisisConfig }) => {
    ...
    useEffect(() => {
        ...
        const fetchAnalysis = async () => {
            try {
                ...
                setFetchError('Connection to AI Engine failed. Ensure backend is running.');
                setIsParsing(false);
            }
        };
    ...
    if (!article) {
        return <div style={{ color: 'white', padding: '2rem' }}>Error: News article not found for {locationName}</div>;
    }
            ...
            <div className="right-panel ai-parse-panel">
                <div className="panel-header">
                    ...
                    {isParsing ? (
                        ...
                        <div className="status-badge complete">{fetchError ? 'Error' : 'Analysis Complete'}</div>
                    )}
                </div>
                ...
                <div className="terminal-window">
                    <div className="terminal-header">
                        ...
                        <span className="terminal-title">agent-core@chainlink:~/parser</span>
                    </div>
                    <div className="terminal-content">
                        <div className="log-line">
                            <span className="cmd-prompt">&gt;</span> initiating context extraction pipeline for article ID: {article.id}...
                        </div>
                        <div className="log-line">
                            <span className="cmd-prompt">&gt;</span> running zero-shot supply chain impact analysis...
                        </div>
                        <div className="log-line">
                            <span className="cmd-prompt">&gt;</span> constructing structured payload via MiniMax...
                        </div>

                        {fetchError ? (
                            <div className="log-line" style={{ color: 'var(--color-red)', marginTop: '1rem' }}>
                                {fetchError}
                            </div>
                        ) : (
                        ...
                        {!isParsing && !fetchError && parsedData && (
                            <div className="log-line success slide-down-fast">
                                <CheckCircle2 size={16} /> severity scored as: <strong style={{ color: 'var(--color-red)', marginLeft: '4px' }}>{parsedData.computedSeverity}</strong>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="action-footer" style={{ padding: 0 }}>
                    <button
                        ...
                        style={{ opacity: (isParsing || !!fetchError) ? 0.5 : 1, width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.75rem' }}
                    >
